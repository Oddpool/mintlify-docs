...
<ResponseField name="mode" type='"block" | "flag"' default='"block"'>
  In block mode, callbacks/hooks raise `InjectionDetectedError` on detection. In flag mode, they log a warning and let the call proceed. See [Modes](/concepts/modes).
</ResponseField>
...
The SDK logs to the `benchspan` logger. Attach a handler to see `flag` warnings and debug output:
